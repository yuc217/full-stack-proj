import { toNewPatientEntry, entrySchema } from '../utils';
...
const patientEntries: Patient[] = patients.map(p => {
    const patientInfo = toNewPatientEntry(p) as Patient;
    const validCheck = p.entries?.map(entry => {
        try {
